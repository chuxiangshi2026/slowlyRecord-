<template>
  <view class="wordbank-container">
    <view class="header">
      <text class="title">进阶词库 III</text>
    </view>
    <!-- 目标词库选择 -->
...
import { loadWordBankD, WORDBANK_D_IDS } from './loaderD'
...
onMounted(async () => {
  ...
  wordbanks.value = WORDBANK_LIST
    .filter(bank => (WORDBANK_D_IDS as string[]).includes(bank.id))
...
const handleAction = async (bank: LocalBankInfo) => {
  ...
  try {
    const words = loadWordBankD(bank.id as any)
